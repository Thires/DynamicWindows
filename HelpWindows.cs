...
namespace DynamicWindows
{
    public class HelpWindows
    {
        ...
        // Quick reference shown by the options-window Help button. Dot leaders line the columns up
        // when rendered in a monospaced font; it's plain selectable text like the other help.
        public string CommandHelp => BuildCommandHelp();

        private static string BuildCommandHelp()
        {
            var windows = new (string Label, string Cmd)[]
            {
                ("aim timer",                    "toggle aimtimer"),
                ("bank debt window",             "bank debt"),
                ("bank deposit window",          "bank deposit"),
                ("bank transfer window",         "bank transfer"),
                ("bank withdraw window",         "bank withdraw"),
                ("bug window",                   "bug window"),
                ("forgetting magic feats",       "ask <feat npc> about forgetting"),
                ("learning magic feats",         "ask <feat npc> about learning"),
                ("profile window",               "profile /edit"),
                ("shopping window",              "shop window"),
                ("spell choosing window",        "ask <npc> about magic"),
                ("store window",                 "store window"),
                ("tdp planning window",          "tdp plan"),
            };
            var pluginCmds = new (string Cmd, string Desc)[]
            {
                ("Display the last XML seen",              "/dumpxml"),
                ("List open windows and their names",      "/debugwindows"),
                ("Toggle others' injury window",           "/toggleotherinjuries"),
                ("Toggle self injury window",              "/toggleinjuries"),
                ("Open self injury window",                "/injurieswindow"),
            };

            var sb = new StringBuilder();
            sb.AppendLine("WINDOWS                    COMMANDS TO OPEN THE WINDOWS");
            //sb.AppendLine();
            int w1 = windows.Max(x => x.Label.Length) + 2;
            foreach (var (label, cmd) in windows)
                sb.AppendLine("  " + (label + " ").PadRight(w1, ' ') + " " + cmd);

            sb.AppendLine();
            sb.AppendLine("PLUGIN COMMANDS");
            //sb.AppendLine();
            int w2 = pluginCmds.Max(x => x.Cmd.Length) + 2;
            foreach (var (cmd, desc) in pluginCmds)
                sb.AppendLine("  " + (cmd + " ").PadRight(w2, ' ') + " " + desc);
